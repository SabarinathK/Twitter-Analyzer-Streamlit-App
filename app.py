...
consumerKey = "X4M0UqVrFrKyLS6INIEaz4d14"
consumerSecret ="Ab0BwAI2nERxJ9FGReMU3rhY0oG088H2L6kmOiHOhMQdNWkT7j"
accessToken ="1393104237857361920-kVPk5ob8idftkTdT9plvlAkwXyuoHV"
accessTokenSecret = "H8mcbjNygHJVr4AS7TNM98seeFNkCnvgINsAYL4fixVEL"
...
def app():
	...
	st.title("Twitter sentiment Analyzer :fire:")


	activities=["Tweet Analysis","Twitter Data"]
	...
	if choice=="Tweet Analysis":
		...
		st.write("1. Fetches the 5 most recent tweets from ID")
		st.write("2. Sentiment Analysis a displays in Bar Graph")

		raw_text = st.text_area("Enter the exact twitter id (without @)")
		st.markdown("Also..check the Full Data Features..on left corner >")

		Analyze = st.selectbox("Select the Activities",  ["Show Recent Tweets","Visualize the Sentiment Analysis"])


		if st.button("Analyze"):

			
			if Analyze == "Show Recent Tweets":
		...
		if st.button("Show Data"):
			...
			st.write(df)
	st.subheader(' ------------------------Created By :  Sabarinath K ---------------------- :smile:')
